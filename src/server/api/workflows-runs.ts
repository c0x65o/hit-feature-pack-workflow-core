...
export async function GET(request: NextRequest) {
  try {
    ...
    if (!isUserAdmin) {
      ...
      const allowed = await db
        ...
        .where(
          and(
            or(...aclConds)!,
            sql`${workflowAcls.permissions}::jsonb @> ${JSON.stringify([WORKFLOW_PERMISSIONS.WORKFLOWS_VIEW_RUNS])}::jsonb`
          )
        )
        .groupBy(workflowAcls.workflowId);

      allowedWorkflowIds = allowed.map((r: any) => String(r.workflowId)).filter(Boolean);
    ...
    if (search) {
      conditions.push(
        or(
          ...
          like(workflows.slug, `%${search}%`),
          like(workflowRuns.id, `%${search}%`),
          like(workflowRuns.correlationId, `%${search}%`)
        )!
      );
    }
    ...
    const [countRow] = whereClause ? await (countQ as any).where(whereClause) : await countQ;
